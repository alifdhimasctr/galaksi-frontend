...
import { FaGraduationCap, FaVenus, FaMars, FaPhone, FaEnvelope, FaMapMarkerAlt, FaSearch, FaBook, FaFilter } from "react-icons/fa";
...
export default function TentorDatabase() {
  ...
  useEffect(() => {
    async function fetchTentors() {
      try {
        ...
        const response = await axios.get(`${API}/tentor/level/${user.level}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        
        // Parse level dari string JSON menjadi array
        const processedTentors = response.data.map(tentor => ({
          ...tentor,
        ...
        console.error("Error fetching tentors:", err);
      } finally {
        setLoading(false);
      }
    }
  ...
  // Filter tentor berdasarkan pencarian dan level
  const filteredTentors = tentors.filter(tentor => {
    const matchesSearch = tentor.name.toLowerCase().includes(searchTerm.toLowerCase()) || 
                          tentor.university.toLowerCase().includes(searchTerm.toLowerCase());
    
    const matchesLevel = filterLevel === "all" || tentor.level.includes(filterLevel);
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow p-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Database Tentor</h1>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-bold text-gray-800">{filteredTentors.length} Tentor</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-700 mb-2">Tidak ada tentor yang ditemukan</h3>
            <p className="text-gray-500">Coba kata kunci pencarian berbeda atau filter level lain</p>
          </div>
        ) : (
    ...
    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100 hover:shadow-xl transition-shadow duration-300">
      ...
      <div className="bg-gray-50 px-6 py-3 text-center">
        <p className="text-xs text-gray-500">
          Bergabung sejak {new Date(tentor.dateJoin).toLocaleDateString('id-ID', {
            day: 'numeric',
            month: 'long',
            year: 'numeric'
          })}
