...
export default function EditMitraModal({ open, onClose, onSuccess, mitraId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const token = Cookies.get("token");
      await axios.put(`${API}/users/mitra/${mitraId}`, form, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      setMsg(err?.response?.data?.message || "Gagal memperbarui mitra");
    }
  };
...
const Input = ({ icon, type, value, onChange, className, disabled }) => (
  <div className="relative flex gap-2">
    ...
    <input
      ...
      onChange={onChange}
      disabled={disabled}
      className={`pl-10 p-2 w-full border border-gray-400 ${disabled ? "bg-gray-100" : "bg-white"} shadow-xs rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${className}`}
    />
  </div>
);