...
export default function DeleteMitraModal({ open, onClose, mitraId, onSuccess }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = Cookies.get("token");
      await axios.delete(`${API}/users/mitra/${mitraId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      setLoading(false);
      setMsg(err?.response?.data?.message || "Gagal menghapus mitra");
  ...
  return (
    ...
    >
      {loading && !mitra ? (
        <div className="text-center py-4">Memuat data mitra...</div>
      ) : msg ? (
          ...
          {mitra && (
            <div className="bg-red-50 rounded-lg p-4 border border-red-200">
              ...
              <div className="text-center text-sm text-red-600 mt-1">
                {mitra.name} • {mitra.siswa?.length || 0} siswa
              </div>
            </div>
          )}
