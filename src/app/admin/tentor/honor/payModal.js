...
export default function PayModal({ open, onClose, onSuccess, honorId }) {
        ...
        if (open && honorId) {
            const fetchHonor = async () => {
                try {
                    ...
                    setMsg(error.response?.data?.message || "Terjadi kesalahan saat mengambil data honor");
                    toast.error("Gagal mengambil data honor!");
                }
            };
            fetchHonor();
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const token = Cookies.get("token");
            await axios.put(`${API}/honor/payment/${honorId}`, formData, {
                headers: {
                    Authorization: `Bearer ${token}`,
            ...
            toast.success("Pembayaran honor berhasil!");
            onSuccess();
            onClose();
        } catch (error) {
    ...
    return (
        <Modal
            ...
            title="Konfirmasi Pembayaran Honor"
            onClose={onClose}
        >
        {/* tampilkan nama bank dan nomor rekening tentor */}
        {honor && (
            <div className="mb-4">
                ...
                <p className="font-semibold text-gray-800">
                    {honor.tentor.bankName} - {honor.tentor.bankNumber}
                </p>
